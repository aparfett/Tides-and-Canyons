...
if variable == "N2":
    var_name = "N2"
    udsn = dsn.N2.isel(n=t1,xi_rho=x_index).values
    udst = dst.N2.isel(n=t1,xi_rho=x_index).values
    uisn = isn.N2.isel(n=t1,xi_rho=x_index).values
    uist = ist.N2.isel(n=t1,xi_rho=x_index).values
    ussn = ssn.N2.isel(n=t1,xi_rho=x_index).values
    usst = sst.N2.isel(n=t1,xi_rho=x_index).values
    fact = 4 
elif variable == "KE":
    var_name = "KE"
    udsn = dsn.KE.isel(n=t1,xi_rho=x_index).values
    udst = dst.KE.isel(n=t1,xi_rho=x_index).values
    uisn = isn.KE.isel(n=t1,xi_rho=x_index).values
    uist = ist.KE.isel(n=t1,xi_rho=x_index).values
    ussn = ssn.KE.isel(n=t1,xi_rho=x_index).values
    usst = sst.KE.isel(n=t1,xi_rho=x_index).values
    fact = 0 
else:
    ValueError("variable must be 'N2' or 'KE'")
#

sigdsn = dsn.sigma.isel(ocean_time=slice(t1r,t2r),xi_rho=x_index).mean(dim='ocean_time').values
sigdst = dst.sigma.isel(ocean_time=slice(t1r,t2r),xi_rho=x_index).mean(dim='ocean_time').values
sigisn = isn.sigma.isel(ocean_time=slice(t1r,t2r),xi_rho=x_index).mean(dim='ocean_time').values
sigist = ist.sigma.isel(ocean_time=slice(t1r,t2r),xi_rho=x_index).mean(dim='ocean_time').values
sigssn = ssn.sigma.isel(ocean_time=slice(t1r,t2r),xi_rho=x_index).mean(dim='ocean_time').values
sigsst = sst.sigma.isel(ocean_time=slice(t1r,t2r),xi_rho=x_index).mean(dim='ocean_time').values

zdsn = dsn.z_rho.isel(ocean_time=slice(t1r,t2r),xi_rho=x_index).mean(dim='ocean_time').values
zdst = dst.z_rho.isel(ocean_time=slice(t1r,t2r),xi_rho=x_index).mean(dim='ocean_time').values
zisn = isn.z_rho.isel(ocean_time=slice(t1r,t2r),xi_rho=x_index).mean(dim='ocean_time').values
zist = ist.z_rho.isel(ocean_time=slice(t1r,t2r),xi_rho=x_index).mean(dim='ocean_time').values
zssn = ssn.z_rho.isel(ocean_time=slice(t1r,t2r),xi_rho=x_index).mean(dim='ocean_time').values
zsst = sst.z_rho.isel(ocean_time=slice(t1r,t2r),xi_rho=x_index).mean(dim='ocean_time').values
...
if save == 1:
    if percentege == 1:
        fig.savefig(f"{var_name}_{phase_str}_pcdiff.png")
    else:
        fig.savefig(f"{var_name}_{phase_str}.png")
